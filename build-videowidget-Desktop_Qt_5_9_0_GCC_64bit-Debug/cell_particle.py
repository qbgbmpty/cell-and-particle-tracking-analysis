...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 cell_particle.py train --dataset=/home/.../mask_rcnn/data/surgery/ --weights=coco
...
class Cell_ParticleConfig(Config):
...
class Cell_ParticleDataset(utils.Dataset):
    def load_VIA(self, dataset_dir, subset, hc=False):
        ...
        self.add_class("cell_particle", 1, "cell")
        self.add_class("cell_particle", 2, "particle")
        if hc is True:
            for i in range(1,14):
                self.add_class("cell_particle", i, "{}".format(i))
            self.add_class("cell_particle", 14, "arm")
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        if image_info["source"] != "cell_particle":
...
def train(model, *dic):
    """Train the model."""
    # Training dataset.
    dataset_train = Cell_ParticleDataset()
    dataset_train.load_VIA(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = Cell_ParticleDataset()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect cell and particle.')
    ...
    # Configurations
    if args.command == "train":
        config = Cell_ParticleConfig()
    else:
        class InferenceConfig(Cell_ParticleConfig):
