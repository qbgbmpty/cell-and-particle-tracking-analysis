...
def save_image(image, image_name, boxes, masks, class_ids, scores, class_names, filter_classs_names=None,
    ...
    for index, value in enumerate(useful_mask_indices):
        ...
        draw.text((x1, y1), "%d %s" % (value+1, label), (255, 255, 255))

    masked_image.save(os.path.join(save_dir, '%s.png' % (image_name)))

def save_single_image(image, image_name, boxes, masks, class_ids, scores, class_names, filter_classs_names=None,
               scores_thresh=0.1, save_dir=None, mode=0):
    """
        image: image array
        image_name: image name
    ...
    if len(useful_mask_indices) == 0:
        ...
        return
    colors = random_colors(len(useful_mask_indices))
    #print(useful_mask_indices)


    
    bbox_colors = np.array(colors).astype(int) * 255
    ...
    for index, value in enumerate(useful_mask_indices):
        ...
        y1, x1, y2, x2 = boxes[value]
        if mode != 2:
            color = tuple(bbox_colors[index])
            draw.rectangle((x1, y1, x2, y2), outline=color)

        # Label
        
        draw.text((x1, y1), "%d %s" % (value+1, label), (255, 255, 255))
        masked_image.save(os.path.join(save_dir, '%s_%d.png' % (image_name,index+1)))
